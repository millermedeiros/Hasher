...
/*!!
 * Hasher query string / hash query plugin
...
hasher.queryUtils = {
     ...
     * - Alias for `queryUtils.toQueryObject( queryUtils.getQueryString(url) )`
     ...
     * @param {string} [url]    URL to be parsed, default to location.href
     * @return {(string|number|null)}   Parameter value.
     */
...
hasher.getHashQuery = function(){
    return this.queryUtils.getQueryString( this.getHash() ).substr(1);
};
...
hasher.getHashQueryAsObject = function(){
    return this.queryUtils.toQueryObject( this.getHashQuery() );
};
...
hasher.getHashQueryParam = function(paramName){
    return this.queryUtils.getParamValue(paramName, this.getHash() );
};
